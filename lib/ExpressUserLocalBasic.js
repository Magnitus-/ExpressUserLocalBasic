...
function GetEmail(RoutingVars, EmailField) 
{
    if(RoutingVars.Result)
    {
        return RoutingVars.Result[EmailField];
    }
    else if(RoutingVars.Updated && (EmailField in RoutingVars.Updated))
    {
        return RoutingVars.Updated[EmailField];
    }
    else
    {
        return RoutingVars.User[EmailField];
    }
}

var ResponseRoutes = {};

ResponseRoutes['UsersPost'] = function(SendEmail, EmailField) {
    return(function(Req, Res, Next) {
        GetRoutingVars(Req, Res, Next, function(RoutingVars) {
            if(SendEmail)
            {
                var GeneratedVars = RoutingVars['Generated'] ? RoutingVars['Generated'] : null;
                SendEmail(RoutingVars, GetEmail(RoutingVars, EmailField), function(Err) {
                    Res.status(201).end();
                });
            }
            else
...
ResponseRoutes['UserPatch'] = function(SendEmail, EmailField) {
    return(function(Req, Res, Next) {
        GetRoutingVars(Req, Res, Next, function(RoutingVars) {
            if(SendEmail)
            {
...
module.exports = function(Options) {
    ...
    var EmailField = Options && Options.EmailField ? Options.EmailField : 'Email';
    
    return function(Router) {
        Router.post('/Users', ResponseRoutes.UsersPost(SendEmail, EmailField));
        ...
        if(Roles&&Roles.Edit)
        {
            Router.patch('/User/:Field/:ID', ResponseRoutes.UserPatch(SendEmail, EmailField));
            Router.put('/User/:Field/:ID/Memberships/:Membership', ReturnStatus(201));
        }
        Router.patch('/User/Self', ResponseRoutes.UserPatch());
        Router.post('/User/:Field/:ID/Recovery/:SetField', ResponseRoutes.UserPatch(SendEmail, EmailField));
